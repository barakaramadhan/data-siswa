import Card from "../Card/Card";
import TodoList from "../todo/TodoList";
export default function HellowWorld() {
  const props = {
    name: "joko",
    text: "halo saya adalah seorang developer",
  };
  const data ={
    name: "badon",
    text: "saya adalah seorang santi abu dzar",
  }

  const cardData={
    imageUrl : "data:image/png;base64,",
    title : "Card Title menggunakan spread syntax",
    desc : "ini adalah card menggunakan spread syntax",
    btn : "click me"
  }
  return (
    <>
      <HeaderHellowWorld {...props} />
      <DescriptionHellowWorld />
      <FooterHellowWorld {...data} />
      <DescriptionHellowWorld2 deskripsi="hallo broo"/>
      <Card {...cardData}/>
      <TodoList/>

    </>
  );
}
export function HeaderHellowWorld({ text, name }) {
  return (
    <div>
      <h1>{text.toUpperCase()}</h1>
      <h2>{name}</h2>
    </div>
  );
}
export function DescriptionHellowWorld({ deskripsi = "belum di isi" }) {
  return <p>{deskripsi.toLowerCase()}</p>;
}

export function FooterHellowWorld({name, text}) {
  return(
    <div>
      <h2>{name}</h2>
      <p>{text}</p>
    </div>
  )
}
export function DescriptionHellowWorld2({ deskripsi = "belum di isi" }) {
  return <p>{deskripsi.toLowerCase()}</p>;
}

// 



